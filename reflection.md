...
- Give one example of an AI suggestion that was incorrect or misleading (including what the AI suggested and how you verified the result).
  * AI suggestion: f"Attempts left: {attempt_limit - st.session_state.attempts -(1 if submit else 0)}"
  Error: submit variable/keyword isn't defined until line 116, after the st.button() call. That expression was added to the display but the button hasn't been created yet at that point, causing a NameError.
...
- In your own words, explain why the secret number kept changing in the original app.
  * Because we dont have a session state so, the script reruns generating a new secret number each time.
- How would you explain Streamlit "reruns" and session state to a friend who has never used Streamlit?
  * Streamlit is like a whiteboard that gets completely erased and redrawn every time you click anything. On the other hand, session state is like a sticky note, values stored there survive each erase.
- What change did you make that finally gave the game a stable secret number?
  * By making sure random.randint() only runs once on the very first load, and the same value is reused on subsequent rerun.
---
...
* Its very important to undestand the proposing changes AI wants to make before you accept them into your code. Althought, the code might seem to work fine, write multiple test cases, and verify functionality on the front end.